...
4. **Relay its summary** to the user verbatim. Append a one-line "how to continue":
   - After questions: *"Answer inline in the file, then reply here (e.g. 'answered') or re-run the `/product-management` command to continue."*
   - After deliverables: *"Review the deliverables. Hand any one to the `/create-implementation-plan` command to plan implementation."*
5. **Stop.** Do not do the PM's work yourself. Do not read source files into the main context. The PM is the only thing that investigates the codebase (via its own analysts).

## Continuing after answers

The user typically replies in the thread (e.g. *"answered batch 1"*). To advance, **re-invoke this skill** via the `/product-management` command (no args) — it finds the active cycle folder and continues. If the user references answered questions but did not re-invoke, tell them to re-run the `/product-management` command to continue (the skill must load to drive the loop).
...
- **Main context stays clean.** Never pull source files or analyst digests into the main conversation — only the PM's short summary.
- **Stateless rounds.** Every `/product-management` command invocation is one round. State lives in the active cycle folder's `state.md`.
- **No AskUserQuestion.** Clarification is always file-based (the PM writes questions; the user answers inline).
