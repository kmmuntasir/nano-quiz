...
2. **Is it complete?** — Read content, check for stubbed code (`// TODO`, empty handlers, `throw new Error('TODO')`, placeholder responses)
3. **Does it match spec?** — Compare against acceptance criteria
4. **Are tests present?** — Check `backend/tests/` (or co-located `*.test.ts`) for backend; `*.test.ts(x)` next to source for frontend
...
| Task ID | Title | Files |
|---------|-------|-------|
| T1 | Title | backend/src/routes/quiz.ts |

### ⚠️ Partial Tasks

| Task ID | Title | Missing | Notes |
|--------|-------|---------|-------|
| T2 | Title | backend/tests/quiz.test.ts | Tests not written |
